module "aws_vm" {
  ...
  cloud_init = templatefile("${path.module}/cloud-init.aws.yaml.tftpl", {
    notes_image = var.notes_image
    crdb_image  = var.crdb_image
    role        = "init"
    join_addr   = ""
  })
  tags = {
...
module "azure_vm" {
  ...
  ssh_public_key = var.ssh_public_key
  ingress_cidrs  = var.ingress_cidrs
  cloud_init = templatefile("${path.module}/cloud-init.azure.yaml.tftpl", {
    notes_image = var.notes_image
    crdb_image  = var.crdb_image
    join_addr   = module.aws_vm.vm_public_ip
  })
  tags = {
    Stack = var.name
  }
